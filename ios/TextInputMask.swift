    ...
    func setMask(reactNode: NSNumber, mask: String, options: NSDictionary) {
        bridge.uiManager.addUIBlock { (uiManager, viewRegistry) in
            DispatchQueue.main.async {
                ...
                let tailPlaceholder = options["tailPlaceholder"] as? String ?? ""
                ...
                let maskedDelegate = MaskedTextFieldDelegate(primaryFormat: mask, autocomplete: autocomplete, autoskip: autoskip, rightToLeft: rightToLeft, affineFormats: affineFormats, customNotations: customNotations, tailPlaceholder: tailPlaceholder) { (_, value, complete) in
